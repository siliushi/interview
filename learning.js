...
console.log(
  bigSum("2424242424242424234534535", "7567575756756757575675756757")
);
...
const p3 = new Promise((resolve, reject) =>
  setTimeout(reject, 300, "p3 rejected")
);
...
Function.prototype.myBind = function (context, ...args) {
  ...
  return function (...newArgs) {
    fn.myApply(context, [...args, ...newArgs]);
  };
};
// 示例
...
greet.myApply(person, ["Hey", "?"]); // 输出: "Hey, Alice?"
Function.prototype.myBind = function (context, ...args) {
  // 保存当前函数的引用
  const fn = this;
...
// 继承
// 1、原型链继承，缺点：共享父类的引用类型数据，修改会影响所有实例。
function Parent() {
  this.name = "Parent";
}
...
function Child() {}
Child.prototype = new Parent(); // 继承自 Parent
Child.prototype.constructor = Child;

const child = new Child();
child.sayHello(); // 输出: Hello from Parent

// 2、借用构造函数（经典继承），缺点: 不能继承父类的原型方法，每个子类实例都有自己的方法。
...
console.log(child.name, child.age); // 输出: Alice 10

// 3、组合继承（原型链 + 借用构造函数）（推荐），优点: 既能继承父类的实例属性，又能继承原型方法。
function Parent(name) {
  this.name = name;
}
Parent.prototype.sayHello = function () {
...
// 6、寄生组合式继承（最佳实践），优点: 解决了原型链继承和构造函数继承的缺点，是 ES5 时代最优的继承方式。
function Parent(name) {
  this.name = name;
}
Parent.prototype.sayHello = function () {
...
// ES6 class 继承 是最推荐的方式，代码清晰，符合现代 JavaScript 规范。
// 寄生组合式继承 是 ES5 时代最优化的继承方式。
// 组合继承 也是不错的选择，但会调用两次父类构造函数，稍微浪费一点内存。
...
emitter.emit("message", "This will not be received"); // 无输出

emitter.once("onceEvent", (msg) => console.log("Once:", msg));
emitter.emit("onceEvent", "First call"); // 输出: Once: First call
emitter.emit("onceEvent", "Second call"); // 无输出
...
function toTousands(num) {
  ...
  return f * integer + res + (decimal && "." + decimal);
}
  ...
  for (var i = 0; i <= n; i++) {
    for (var w = 0; w <= capacity; w++) {
      if (i == 0 || w == 0) {
        ...
        K[i][w] = max1(value[i - 1] + K[i - 1][w - size[i - 1]], K[i - 1][w]);
      } else {
        K[i][w] = K[i - 1][w];
      }
    }
...
// 泡茶/泡咖啡
function A() {}
A.prototype.boilwater = function () {
  console.log("把水煮沸");
};
A.prototype.brew = function () {
  throw new Error("需要在子类里实现具体方法");
};
A.prototype.isZL = function () {
  return true;
}; // 默认需要佐料，可以在
...
let item = a.find((i) => i.name === "1");
item.name = "5";
// 2、map
a = a.map((i) => (i.name === "2" ? { ...i, name: "6" } : i));
console.log(a);
...
function random7() {
  const a = random5(); // 1-5
  const b = random5(); // 1-5
  let num = (a - 1) * 5 + b; // 1-25
  if (num <= 21) renturn(num % 7) + 1;
}
function random10() {
  const a = random7(); // 1-7
  const b = random7(); // 1-7
  const num = (a - 1) * 7 + b; // 0-42 + 1-7  = 1-49
  if (num <= 40) {
...
function formatCardNumber(cardNum) {
  return cardNum.replace(/\d{4}(?=\d)/g, "$& "); // 每4位后面加空格（如果后面还有数字）
}
// 实现jsonp
function jsonp(url, callback) {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = function () {
      ...
      document.body.removeChild(scrpit);
      delete window[callback];
    };
...
// 一次可以走一步或者两步，n个阶梯的楼梯有多少种走法，[1,2,3,5]
// 实现扫雷（二维数组，随机分布地雷坐标）
// 计算累进税率
